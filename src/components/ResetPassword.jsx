...
const ResetPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (password !== confirmPassword) {
            dispatch(showAlert({ message: 'Password do not match!', type: 'danger' }))
            return;
        }

        try {
            const res = await fetch('http://localhost:5500/auth/reset-password', {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                dispatch(showAlert({ message: result.message, type: 'danger' }))
                return;
            }
            dispatch(showAlert({ message: 'Password reset successfully', type: 'success' }))
            navigate('/auth/login');
        } catch (err) {
